...
test('visualizar OS abre modal sem tela branca', async ({ browser }) => {
  ...
  await expect(page.getByRole('button', { name: /Imprimir/i })).toBeVisible({ timeout: 15000 });
  await expect(page.getByRole('heading', { name: /Linha operacional/i })).toBeVisible({ timeout: 15000 });
  await expect(page.getByRole('heading', { name: /Equipamento/i })).toBeVisible({ timeout: 15000 });

  expect(pageErrors, `A página lançou erros ao abrir o modal: ${pageErrors.join(' | ')}`).toEqual([]);

  await page.screenshot({
    path: 'test-results/service-orders-viewer.png',
    fullPage: true,
  });
