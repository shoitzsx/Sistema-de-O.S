...
const username = `editnopass_${Date.now()}`;

const createResponse = await fetch(`${siteUrl}/api/admin/users`, {
  method: 'POST',
  headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${accessToken}` },
  body: JSON.stringify({ name: 'Usuario Edit Sem Senha', username, password: '1234', role: 'operator', allowed_modules: [1] }),
});
...
const updateResponse = await fetch(`${siteUrl}/api/admin/users`, {
  method: 'PUT',
  headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${accessToken}` },
  body: JSON.stringify({ id: userId, name: 'Usuario Editado Sem Trocar Senha', username, role: 'operator', allowed_modules: [1,2,3] }),
});
